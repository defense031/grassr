# augment_empirical_bands_k2_q097.R --- APPEND q=0.97 entries to the
# existing k=2 portion of empirical_q_hat_surface in grass/R/sysdata.rda.
#
# Background: the k=2 surface (post 2026-05-10 fix) has q_true grid
# {0.55, 0.60, ..., 0.95, 0.99}. Panels with q_hat in (0.96, 0.98) snap
# to q_true=0.99 (where the empirical distribution is tightly above q_hat)
# and clamp to percentile 0. Adding q_true=0.97 closes that snap gap.
#
# This script is additive: it reads the current sysdata, finds k=2 rows,
# appends 156 new k=2 q=0.97 rows from the augmentation sim, and writes
# back. It does not touch the k>=3 portion of the surface.
#
# Run from PABAK_Investigation project root (after the augmentation sim
# completes):
#   Rscript grass/data-raw/augment_empirical_bands_k2_q097.R
#
# Inputs:
#   paper1_2_merged/output/multirater_sim_k2_q097_aug/summaries.rds
#       (156 x 22)
#   paper1_2_merged/output/multirater_sim_k2_q097_aug/per_scenario/scenario_NNNN.rds
#   paper2/code/04_reference_closed_form.R   (cf_panel)
#   paper2/code/06_grid.R                    (F_params_for, e_p_logit_normal)
# Outputs:
#   grass/R/sysdata.rda  (updated --- existing entries preserved, 156 added)
...
AUG_DIR <- "paper1_2_merged/output/multirater_sim_k2_q097_aug"
...
# Offset: +210000 to keep distinct from base k=2 (+200000) and k=25 (+100000).
AUG_SCENARIO_ID_OFFSET <- 210000L
...
existing_k2 <- existing_eqs$index[existing_eqs$index$k == 2L, ]
cat(sprintf("  k=2 q-grid: {%s}\n",
            paste(sort(unique(existing_k2$q_true)), collapse = ",")))
if (0.97 %in% existing_k2$q_true) {
  cat("WARNING: q=0.97 already present in k=2 grid; existing rows will be replaced.\n")
}

# ---- Load augmentation summaries + per_rep --------------------------------
cat("Loading q=0.97 augmentation summaries...\n")
s <- readRDS(file.path(AUG_DIR, "summaries.rds"))
cat(sprintf("  %d scenarios, k=%d, q=%s, N in {%s}, F-keys=%d\n",
            nrow(s), unique(s$k),
            paste(sort(unique(s$q)), collapse = ","),
            paste(sort(unique(s$N)), collapse = ","),
            length(unique(s$F_key))))
stopifnot(unique(s$k) == 2L)
stopifnot(unique(s$q) == 0.97)
...
# Drop any prior k=2 q=0.97 rows (defensive; expected no-op).
keep_existing <- !(existing_idx$k == 2L & existing_idx$q_true == 0.97)
n_dropped <- sum(!keep_existing)
if (n_dropped > 0L) {
  cat(sprintf("Dropping %d existing k=2 q=0.97 rows\n", n_dropped))
}
...
empirical_q_hat_surface <- list(
  ...
  source    = sprintf(
    "multirater_sim_v3_combined_full.rds + multirater_sim_k2_extension + multirater_sim_k2_q097_aug (%s scenarios; k in {%s})",
...
cat(sprintf("Final empirical_q_hat_surface: %d scenarios, k in {%s}\n",
            nrow(merged_index),
            paste(sort(unique(merged_index$k)), collapse = ",")))
new_k2 <- merged_index[merged_index$k == 2L, ]
cat(sprintf("  k=2 q-grid: {%s} (%d scenarios)\n",
            paste(sort(unique(new_k2$q_true)), collapse = ","), nrow(new_k2)))
