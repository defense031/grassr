# augment_empirical_bands_q094_k2_k25.R --- APPEND q=0.94 entries to
# both the k=2 and k=25 portions of empirical_q_hat_surface.
#
# Background: q=0.94 fills the midpoint gap between the q=0.92 and
# q=0.95 cohorts that previously left tab:card_summary row 6 (k=25,
# PABAK=0.78, q_hat=0.9416) clamped to percentile 0. The q_hat=0.94
# panels at k=25 will now snap to the q=0.94 cohort, returning a
# centered percentile near 50. The k=2 q=0.94 grid point is added
# for symmetry — no current paper anchor depends on it, but it
# guards against future practitioner panels landing in the same gap.
#
# Run from project root:
#   Rscript grass/data-raw/augment_empirical_bands_q094_k2_k25.R
#
# Inputs:
#   paper1_2_merged/output/multirater_sim_q094_aug/summaries.rds (312 x 22)
#   paper1_2_merged/output/multirater_sim_q094_aug/per_scenario/scenario_NNNN.rds
#   paper2/code/04_reference_closed_form.R + 06_grid.R
# Outputs:
#   grass/R/sysdata.rda  (existing entries preserved; 312 added)
...
AUG_DIR <- "paper1_2_merged/output/multirater_sim_q094_aug"
...
# Offset: +230000 to keep distinct from prior offsets.
AUG_SCENARIO_ID_OFFSET <- 230000L
...
cat("Loading q=0.94 augmentation summaries (k=2,25)...\n")
s <- readRDS(file.path(AUG_DIR, "summaries.rds"))
cat(sprintf("  %d scenarios, k in {%s}, q in {%s}, N in {%s}, F-keys=%d\n",
            nrow(s),
            paste(sort(unique(s$k)), collapse = ","),
            paste(sort(unique(s$q)), collapse = ","),
            paste(sort(unique(s$N)), collapse = ","),
            length(unique(s$F_key))))
stopifnot(setequal(unique(s$k), c(2L, 25L)))
stopifnot(unique(s$q) == 0.94)
...
# Defensive: catches the per_rep ordering bug class.
stopifnot(setequal(names(per_rep), as.character(s$scenario_id)))
stopifnot(!any(duplicated(names(per_rep))))

# ---- Closed-form lookup tables (one per (F_family, F_key, k)) -------------
uniq_Fk <- unique(s[, c("F_family", "F_key", "k")])
cat(sprintf("Building %d closed-form lookup tables (k=2 + k=25)...\n",
            nrow(uniq_Fk)))
...
# Drop any prior q=0.94 rows at k in {2, 25} (defensive; expected no-op).
keep_existing <- !(existing_idx$k %in% c(2L, 25L) & existing_idx$q_true == 0.94)
n_dropped <- sum(!keep_existing)
if (n_dropped > 0L) {
  cat(sprintf("Dropping %d existing q=0.94 rows at k in {2, 25}\n", n_dropped))
}
...
empirical_q_hat_surface <- list(
  ...
  source    = sprintf(
    "multirater_sim_v3_combined_full.rds + multirater_sim_k2_extension + multirater_sim_k2_q097_aug + multirater_sim_k25_q092_097_aug + multirater_sim_q094_aug (%s scenarios; k in {%s})",
...
cat(sprintf("Final empirical_q_hat_surface: %d scenarios, k in {%s}\n",
            ...
            paste(sort(unique(merged_index$k)), collapse = ",")))
for (kk in c(2L, 25L)) {
  sub <- merged_index[merged_index$k == kk, ]
  cat(sprintf("  k=%2d q-grid: {%s} (%d scenarios)\n",
              kk, paste(sort(unique(sub$q_true)), collapse = ","), nrow(sub)))
}
