...
S6B <- file.path(ROOT, "grassr/simulation/v070_program/tier1/stage6b_topup")
...
cat(sprintf("stage6b | split cells: %d | todo: %d | draws: %s\n",
...
invisible(future_lapply(seq_len(nrow(todo)), function(i) {
  row <- todo[i, ]; set.seed(20260708L + row$cell_id)
