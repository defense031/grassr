...
Two common clinical designs do not fit this matrix directly, and
the difference matters because the whole calibration assumes every
...
- **Repetitions within a session.** A patient performs the task
  ...
  the repetitions as rows. Stacked repetitions tell the package you
  have three times as many subjects as you do, and because
  repetitions of one patient are correlated, the matrix carries less
  information than its row count claims. The reported band would
  come out narrower than the design earned. Build one matrix per
  repetition and compare the cards, or fix one repetition per
  patient by protocol before building the matrix.
- **The same patients across sessions over time.** Each session is a
  valid panel on its own, because within a session the rows are
  distinct patients. Score one card per session; the sequence of
  cards is the stability record, read on a common design-conditioned
  scale. What the package does not do is pool the sessions into one
  number or split the disagreement into patient, session, and rater
  components. That variance-components question is real, and it is
  outside this package's calibration.

The package cannot detect either design from the matrix itself,
since a 0/1 matrix does not say which rows share a patient. This
part of the contract is enforced by you, not by an error message.
