...
- **Bug fix (0.5.2):** the k = 2 portion of the bundled
  `empirical_q_hat_surface` had a build-time scenario mislabeling
  that affected roughly half of all k = 2 sysdata cells in v0.5.0
  and v0.5.1. A corrected k = 2 sub-grid plus high-q augmentations
  at k = 2 and k = 25 are bundled here; full details and migration
  impact in `NEWS.md`. Anyone who computed k = 2 surface percentiles
  on v0.5.0 / v0.5.1 should re-run on v0.5.2.
- **Report Card render (0.5.2):** under the `aligned` and `caution`
  flags, `print(grass_report(Y))` now lists every panel coefficient
  with its surface percentile (previously only the primary
  printed). The primary still carries the `<- primary` marker and
  the qualifier (`decisive` / `moderate` / `weak`) inline. ICC
  carries a `[distribution-sensitive]` marker in the Report Card
  rather than only in the asymmetry-panel print.
- **Scope locked (0.5.0):** δ̂ is computed over the four-coefficient
  agreement family (PABAK, AC1, Fleiss κ, Krippendorff α). ICC is
  reported alongside on the Report Card with a
  `[distribution-sensitive]` marker but does not enter δ̂ by
  construction; its reference surface depends on the full subject-
  prevalence distribution rather than on `(q, pi_hat)` alone.
- **Per-(k, N) δ̂ thresholds (0.5.0):** `delta_thresholds_lookup`
  bundles the size-α-calibrated (caution, divergent) pair per
  design cell; at off-grid `(k, N)` the lookup snaps to the
  nearest fully-calibrated cell and surfaces the snap on the
  Report Card.
- **Fitted-ICC reference (0.5.0):** rebuilt from a unified
  Julia/MixedModels.jl sim across k ∈ {3, 5, 8, 15, 25} × N ∈
  {30, 50, 75, 100, 200, 300, 500, 1000} on a 14-point q-grid.
  Empirical clamp rate at common applied designs dropped from
  98.2% (v0.3 baseline) to 7.7%.
...
- **`delta`** is the cross-coefficient percentile spread in pp across
  the agreement family (PABAK, AC1, Fleiss kappa, Krippendorff alpha;
  ICC excluded by construction). The caution and divergent cuts are
  size-α calibrated per `(k, N)` (`delta_thresholds_lookup`); at the
